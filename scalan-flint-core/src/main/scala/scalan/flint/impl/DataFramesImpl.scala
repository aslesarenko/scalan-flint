...
!!!("Cannot convert from DataFrame to PairDF: missing fields List(left, right)")
  ...
  class ShardedDFElem[T](val iso: Iso[ShardedDFData[T], ShardedDF[T]])(implicit eT: Elem[T])
    ...
    override def convertDataFrame(x: Rep[DataFrame[T]]) = // Converter is not generated by meta
!!!("Cannot convert from DataFrame to ShardedDF: missing fields List(distrib, shards)")
  ...
  class ShardedViewDFElem[T](val iso: Iso[ShardedViewDFData[T], ShardedViewDF[T]])(implicit eT: Elem[T])
    ...
    override def convertDataFrame(x: Rep[DataFrame[T]]) = // Converter is not generated by meta
!!!("Cannot convert from DataFrame to ShardedViewDF: missing fields List(view)")
...
trait DataFramesExp extends ScalanExp with DataFramesDsl {
  ...
  object ShardedDFMethods {
    ...
    object join {
      def unapply(d: Def[_]): Option[(Rep[ShardedDF[T]], DF[I], Rep[T => K], Rep[I => K], Rep[Int], Rep[Int]) forSome {type T; type I; type K}] = d match {
        case MethodCall(receiver, method, Seq(innerRdd, outerKey, innerKey, estimation, kind, _*), _) if receiver.elem.isInstanceOf[ShardedDFElem[_]] && method.getName == "join" =>
          Some((receiver, innerRdd, outerKey, innerKey, estimation, kind)).asInstanceOf[Option[(Rep[ShardedDF[T]], DF[I], Rep[T => K], Rep[I => K], Rep[Int], Rep[Int]) forSome {type T; type I; type K}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[ShardedDF[T]], DF[I], Rep[T => K], Rep[I => K], Rep[Int], Rep[Int]) forSome {type T; type I; type K}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object getShard {
      def unapply(d: Def[_]): Option[(Rep[ShardedDF[T]], Rep[Int]) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(node, _*), _) if receiver.elem.isInstanceOf[ShardedDFElem[_]] && method.getName == "getShard" =>
          Some((receiver, node)).asInstanceOf[Option[(Rep[ShardedDF[T]], Rep[Int]) forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[ShardedDF[T]], Rep[Int]) forSome {type T}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object gather {
      def unapply(d: Def[_]): Option[Rep[ShardedDF[T]] forSome {type T}] = d match {
        case MethodCall(receiver, method, _, _) if receiver.elem.isInstanceOf[ShardedDFElem[_]] && method.getName == "gather" =>
          Some(receiver).asInstanceOf[Option[Rep[ShardedDF[T]] forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Rep[ShardedDF[T]] forSome {type T}] = exp match {
  ...
  object ShardedViewDFMethods {
    object join {
      def unapply(d: Def[_]): Option[(Rep[ShardedViewDF[T]], DF[I], Rep[T => K], Rep[I => K], Rep[Int], Rep[Int]) forSome {type T; type I; type K}] = d match {
        case MethodCall(receiver, method, Seq(innerRdd, outerKey, innerKey, estimation, kind, _*), _) if receiver.elem.isInstanceOf[ShardedViewDFElem[_]] && method.getName == "join" =>
          Some((receiver, innerRdd, outerKey, innerKey, estimation, kind)).asInstanceOf[Option[(Rep[ShardedViewDF[T]], DF[I], Rep[T => K], Rep[I => K], Rep[Int], Rep[Int]) forSome {type T; type I; type K}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[ShardedViewDF[T]], DF[I], Rep[T => K], Rep[I => K], Rep[Int], Rep[Int]) forSome {type T; type I; type K}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object getShard {
      def unapply(d: Def[_]): Option[(Rep[ShardedViewDF[T]], Rep[Int]) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(node, _*), _) if receiver.elem.isInstanceOf[ShardedViewDFElem[_]] && method.getName == "getShard" =>
          Some((receiver, node)).asInstanceOf[Option[(Rep[ShardedViewDF[T]], Rep[Int]) forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[ShardedViewDF[T]], Rep[Int]) forSome {type T}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object gather {
      def unapply(d: Def[_]): Option[Rep[ShardedViewDF[T]] forSome {type T}] = d match {
        case MethodCall(receiver, method, _, _) if receiver.elem.isInstanceOf[ShardedViewDFElem[_]] && method.getName == "gather" =>
          Some(receiver).asInstanceOf[Option[Rep[ShardedViewDF[T]] forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Rep[ShardedViewDF[T]] forSome {type T}] = exp match {
  ...
  object DataFrameMethods {
    ...
    object getShard {
      def unapply(d: Def[_]): Option[(Rep[DataFrame[T]], Rep[Int]) forSome {type T}] = d match {
        case MethodCall(receiver, method, Seq(node, _*), _) if receiver.elem.isInstanceOf[DataFrameElem[_, _]] && method.getName == "getShard" =>
          Some((receiver, node)).asInstanceOf[Option[(Rep[DataFrame[T]], Rep[Int]) forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[DataFrame[T]], Rep[Int]) forSome {type T}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
    ...
    object gather {
      def unapply(d: Def[_]): Option[Rep[DataFrame[T]] forSome {type T}] = d match {
        case MethodCall(receiver, method, _, _) if receiver.elem.isInstanceOf[DataFrameElem[_, _]] && method.getName == "gather" =>
          Some(receiver).asInstanceOf[Option[Rep[DataFrame[T]] forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Rep[DataFrame[T]] forSome {type T}] = exp match {
    ...
    object nShards {
      def unapply(d: Def[_]): Option[Rep[DataFrame[T]] forSome {type T}] = d match {
        case MethodCall(receiver, method, _, _) if receiver.elem.isInstanceOf[DataFrameElem[_, _]] && method.getName == "nShards" =>
          Some(receiver).asInstanceOf[Option[Rep[DataFrame[T]] forSome {type T}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[Rep[DataFrame[T]] forSome {type T}] = exp match {
